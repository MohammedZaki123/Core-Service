...
function rbac(option) {
    return async (req, res, next) => {
        ...
        try {
            ...
            if (req.user?.role == enums_1.SystemRole.SYSTEM_ADMIN) {
                if (allowSystemAdmin) {
                    return next();
                }
            ...
            if (req.user?.role == enums_1.SystemRole.RESTAURANT_USER) {
                const permissions = await permission_cache_service_1.permissionCacheService.getPermissions(req.user?.restaurantRole);
                const permissionExist = permission_cache_service_1.permissionCacheService.hasPermission(permissions, resource, action);
                if (!permissionExist) {
                    return res.status(403).json({
                        error: "Permission denied",
                    });
...
function requireRestaurantMember(paramName = 'restaurantId') {
    return async (req, res, next) => {
        ...
        const restaurantId = (0, validate_1.validatePathParameter)(req.params[paramName], "Restaurant ID");
        if (req.user?.role == enums_1.SystemRole.SYSTEM_ADMIN) {
            return next();
        }
...
function requireBranchAccess(paramName = 'branchId') {
    return async (req, res, next) => {
        try {
            ...
            if (req.user?.role == enums_1.SystemRole.SYSTEM_ADMIN || req.user?.restaurantRole == "owner") {
                return next();
            }
