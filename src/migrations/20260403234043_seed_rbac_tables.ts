...
export async function up(knex: Knex): Promise<void> {
    // Insert predefined roles (only 3 roles)
    await knex.raw(`
    ...
    await knex.raw(`
       ...
       await knex.raw(`
        ...
        `);
         // Branch Manager permissions - product CRUD, branch update, member read,

    await knex.raw(`INSERT INTO role_permissions(role_id, permission_id, created_at)
                        SELECT r.id, p.id, NOW() FROM roles r, permissions p 
                        WHERE r.name = 'branch_manager' AND p.resource || ':' || p.action IN (
                      'core:product:create',
                      'core:product:read',
                      'core:product:update',
                      'core:member:read',
