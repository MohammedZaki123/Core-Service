...
#### Get User Info (with saved cookies)
```bash
...
```bash
curl -X POST http://localhost:3000/api/restaurants/1/branches \
  -H "Content-Type: application/json" \
  -b cookies.txt \
  -d '{
    "label": "Downtown Branch",
...
#### Get Nearby Branches
```bash
curl -X GET "http://localhost:3000/api/branches/nearby?lat=30.043304&lng=31.200782&radius=5" \
  -H "Content-Type: application/json"
```
...
async function login() {
  ...
  // Cookies are automatically saved by browser
}
```
...
async function getUserInfo() {
  const response = await fetch('http://localhost:3000/api/users/me', {
    ...
    credentials: 'include' // Include saved cookies
  });
...
async function createAddress(address) {
  const response = await fetch('http://localhost:3000/api/customer/addresses', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
```bash
curl http://localhost:3000/api/restaurant
```

#### Get Nearby Branches (Public)
```bash
curl "http://localhost:3000/api/branches/nearby?lat=30.043304&lng=31.200782"
```

#### Get Restaurant Categories (Public)
```bash
curl http://localhost:3000/api/restaurants/1/categories
```
...
```json
{
  "email": "owner@quick-bite.com",
  "phone": "01012345678",
  "name": "Ahmed Hassan",
  "password": "QuickBite123",
  "role": "restaurant_user",
  "restaurant": {
    "name": "Ahmed's Restaurant",
    "primaryCountry": "EG",
    "logoURL": "https://example.com/ahmed-logo.png"
  }
}
```
...
```json
{
  "label": "Zamalek Branch",
  "countryCode": "EG",
  "lat": 30.0597,
  "lng": 31.1874,
  "addressText": "Zamalek Island, Cairo",
  "opensAt": "10:00",
  "closesAt": "01:00",
  "currency": "EGP",
  "deliveryRadius": 8
...
```json
{
  ...
  "description": "Traditional Egyptian street food with pasta, lentils, and tomato sauce",
...
- **Cause**: Missing member or invalid member ID
- **Solution**: Verify member ID exists before making requests
...
- **Solution**: Verify user role and restaurant membership

#### 4. "Restaurant not found"
- **Cause**: Invalid restaurant ID
- **Solution**: Verify restaurant exists using GET /restaurant/:id

#### 5. "Email already exists"
- **Cause**: Email already registered
- **Solution**: Use different email or reset password if forgot credentials
...
- **Email**: api-support@quick-bite.com
- **Documentation**: This file
- **Issues**: Include Correlation ID from response headers
